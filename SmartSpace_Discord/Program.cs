...
namespace SmartSpace_Discord
{
        class Program
        {
            ...
            {
                await client.LoginAsync(TokenType.Bot, "NzA3NzI5NzA5MjM2MjI0MDgz.XteGDw._Ez936or_L7JOtMwtVmrWHLDNfk"); // 봇 로그인
            ...
            private async Task MessageReceivedAsync(SocketMessage message) // SocketMessage - Type을 의미, message
            {
                ...
                if(message.Content == "!ping") // 유저가 !ping이라는 명령어를 디스코드에 입력하게 되면 
                {
                     await message.Channel.SendMessageAsync("pong!"); // pong!을 반환하게 한다.
                }
        }
        }
}
